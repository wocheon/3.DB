...
/* ENABLE_PARALLEL HINT 관련 */
- ENABLE_PARALLEL_DML 
 : ALTER SESSION ENABLE PARALLEL DML; 과 동일한 기능 (TM LOCK + PARALLEL ) 
   EX) INSERT /*+ ENABLE_PARALLEL_DML PARALLEL(16) */ INTO TEST_TBL
		SELECT * FROM DUAL;
   * 대용량 테이블의 경우 INDEX를 타는 것보다 FULL SCAN 이 나을수 있음 
...
형식 : 	
	...
	EX) CREATE TABLE TEST_TBL1 (ID NUMBER, NAME VARCHAR2(10));
...
/* LPAD, LTRIM  */		
	LPAD : 왼쪽부터 특정 자리수만큼 특정문자를 삽입
	EX)  SELECT LPAD( '1', 3, '0') FROM DUAL; => 001
	
	LTRIM : 왼쪽부터 반복되는 특정문자 삭제 
	EX) SELECT LTRIM(LPAD( '1', 3, '0'),'0') FROM DUAL; => 1
...
* LOAD AS SELECT (DIRECT PATH LOAD)
	: SQL 문장을 GENERATE 하여 사용 X 
	  BIND ARRAY BUFFER를 사용하지않고 메모리에 DATA BLOCK을 만들어 데이터를 넣은 후 그대로 WRITE
	  LOAD 시작시 TABLE LOCK 을 검. REDU LOG FILE 불필요.
	  UNDO X(ROLLBACK 불가), 병렬사용시 성능 향상 가능.
...
SET AUTOTRACE ON; ( 실제 수행 및 출력 )
OR 
SET AUTOTRACE TRACEONLY; ( 실제수행 X 출력 O )
...
purge recyclebin; (본인 계정의 $BIN 테이블만 삭제)
purge dba_recyclebin; (모든 $BIN 테이블 삭제 - SYS로만 가능)
purge tablespace TS_OWN_D user own; ( 특정 스키마에 포함된 $BIN 테이블중 특정 테이블스페이스를 지정하여 삭제.)


/* 테이블 스페이스 MOVE */
ALTER TABLE OWN.TEST_TBL1 ;

ALTER TABLE TEST_TBL1 MOVE TABLESPACE TS_TEST_D;

--partition 테이블 move 방법 
...
alter tablE [ table_name ] move ONLINE
 : MOVE => SEGMENT_SIZE를 REORG 하는 작업. 
		   DELETE를 한다고해도 사용중인 SEGMENT_SIZE가 변하진않음 (SHINK)
		   그러므로 비어있는만큼 SEGMENT_SIZE를 줄일때 해당 방식을 사용.
		   단 ONLINE을 사용하지않으면 TM LOCK이 발생함.
		   완료 후에는 인덱스가 INVALID 상태가 되므로, ALTER INDEX [ 인덱스명 ] REBUILD 구문을 추가로 실행 
		   ( ROWID가 변경되어 발생하는 현상)
   
   MOVE ONLINE => 동일한 형태로 작업이 진행되나 TM락을 거는 것을 방지함. + 자체적으로 INDEX REBUILD 수행.
...
from 
 ...
 /*  데이터파일 이름변경  */ 
-- TABLESPACE를 OFFLINE으로 변경  => 변경할 데이터 파일 복사 혹은 이름 변경 (cp or mv)
-- => alter 문을 통해  데이터파일명을 재설정 => TABLESPACE ONLINE으로 변경 
