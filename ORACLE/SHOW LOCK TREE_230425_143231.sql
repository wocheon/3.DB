...
,decode(v.wait_sid, null, ' ' , '^' , ' ☆', '(' || v.inst_id || ')' || v.waid_sid )   "Wait SID"
,v. gb  "HW type"
,v.inst_id "Instance"
...
6,' Ex',
to_char( v.request) )
,(select object_name || ' (' || substr(object_type ,1, 1) || ') ' from dba_objects 
do where do.object_id = s.row_wait_obj#) locked_obj
,substr(s.username, 1,8)
...
,decode(substr(s.action,1,4) , 'FRM:', s.module || ' (Form) ', 'Onli', s.module || ' (Form)', 'Conc', s.module || ' (Conc) ', s.module )
...
/* TIBERO */
select 
case when S.SID = W.WAIT_SID then 'WAIT(' || HOLD_SID ||')' 
else ' ' end "HOLD"
,S.STATE
